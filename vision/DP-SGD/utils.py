...
def get_data_loader(dataset, batchsize):
    if dataset == "svhn":
        ...
        )  # load full btach into memory, to concatenate with extra data
...
def loop_for_sigma(q, T, eps, delta, cur_sigma, interval, rdp_orders=32, rgp=True):
    while True:
        ...
        if rgp:
            rdp = (
                compute_rdp(q, cur_sigma, steps, orders) * 2
            )  ## when using residual gradients, the sensitivity is sqrt(2)
        else:
...
def get_sigma(q, T, eps, delta, init_sigma=10, interval=1.0, rgp=True):
    ...
    cur_sigma, _ = loop_for_sigma(q, T, eps, delta, cur_sigma, interval, rgp=rgp)
    interval /= 10
    cur_sigma, previous_eps = loop_for_sigma(
        q, T, eps, delta, cur_sigma, interval, rgp=rgp
    )
    return cur_sigma, previous_eps
