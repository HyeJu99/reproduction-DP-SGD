...
class Kalman:
    ...
    # 기본 데이터 구성
    def make_data(self):
        true_data = np.array(self.data.cpu())
        observed_data = true_data + np.random.normal(0, 0.5, true_data.shape)
        return torch.tensor(observed_data).to("cuda")
    ...
    def kalman_filter(self):
        ...
        for k in range(1, ts_length):
            ...
            K[k, :] = (
                Pminus[k, :, :]
                @ self.H.T
                @ np.linalg.inv(self.H @ Pminus[k, :, :] @ self.H.T + self.R)
            )
            xhat[k, :] = xhatminus[k, :] + K[k, :] @ (
                self.z[k] - self.H @ xhatminus[k, :]
            )
            P[k, :, :] = (
                np.eye(dim_state) - K[k, :][: np.newaxis] @ self.H[np.newaxis, :]
            ) @ Pminus[k, :, :]
    ...
    def kalman_smoother(self, xhat, xhatminus, P, Pminus):
        ...
        for k in range(ts_length - 2, -1, -1):
            J = (
                P[k, :, :] @ self.A.T @ np.linalg.inv(Pminus[k + 1, :, :])
            )  # 스무딩 이득 계산
            xhat_smooth[k, :] = xhat[k, :] + J @ (
                xhat_smooth[k + 1, :] - xhatminus[k + 1, :]
            )
            P_smooth[k, :, :] = (
                P[k, :, :] + J @ (P_smooth[k + 1, :, :] - Pminus[k + 1, :, :]) @ J.T
            )
    ...
    def forecast(self, xhat, P, A, steps):
        ...
        for k in range(1, steps):
            ...
            forecast_covariances[k, :, :] = (
                A @ forecast_covariances[k - 1, :, :] @ A.T + self.Q
            )
