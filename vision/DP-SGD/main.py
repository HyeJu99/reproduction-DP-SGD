...
parser = argparse.ArgumentParser(
    description="Differentially Private learning with DP-SGD"
)
...
parser.add_argument(
    "--lr", default=0.1, type=float, help="base learning rate (default=0.1)"
)
...
parser.add_argument(
    "--private", "-p", action="store_true", help="enable differential privacy"
)
parser.add_argument("--clip", default=5.0, type=float, help="gradient clipping bound")
parser.add_argument("--eps", default=8.0, type=float, help="privacy parameter epsilon")
...
if (
    args.dataset == "svhn"
):  ## For SVHN, we concatenate training samples and extra samples to build the training set.
    trainloader, extraloader, testloader, n_training, n_test = get_data_loader(
        "svhn", batchsize=args.batchsize
    )
    ...
    trainloader, testloader, n_training, n_test = get_data_loader(
        "cifar10", batchsize=args.batchsize
    )
...
print(
    "\n==> Computing noise scale for privacy budget (%.1f, %f)-DP"
    % (args.eps, args.delta)
)
...
def train(epoch):
    ...
    for batch_idx in range(steps):
        if args.dataset == "svhn":
            ...
            inputs, targets = (
                train_samples[current_batch_idxes],
                train_labels[current_batch_idxes],
            )
        else:
    ...
    t1 = time.time()
    print(
        "Train loss:%.5f" % (train_loss / (batch_idx + 1)),
        "time: %d s" % (t1 - t0),
        "train acc:",
        acc,
        end=" ",
